...
IplImage* ViewMainPage::getIplgray(int n)
{
    Qroilib::DocumentView *v = view(n);//currentView();
    const QImage *pimg = v->image();
    if (!pimg)
        return nullptr;
    if (pimg->isNull())
    ...
    IplImage riplImg;
    IplImage *iplImg;
    static IplImage *grayImg[8] = {nullptr, };

    riplImg = frame;
    iplImg = &riplImg;

    CvSize isize = cvSize(iplImg->width, iplImg->height);
    if (grayImg[n] == nullptr)
        grayImg[n] = cvCreateImage(isize, IPL_DEPTH_8U, 1);
    if (iplImg->nChannels == 3)
        cvCvtColor(iplImg, grayImg[n], CV_RGB2GRAY);
    else if (iplImg->nChannels == 4) {
        if (strncmp(iplImg->channelSeq, "BGRA", 4) == 0)
            cvCvtColor(iplImg, grayImg[n], CV_BGRA2GRAY);
        else
            cvCvtColor(iplImg, grayImg[n], CV_RGBA2GRAY);
    } else
        cvCopy(iplImg, grayImg[n]);

    return grayImg[n];
}

IplImage* ViewMainPage::getIplcolor(int n)
{
    Qroilib::DocumentView *v = view(n);//currentView();
    const QImage *pimg = v->image();
    if (!pimg)
    ...
    IplImage riplImg;
    IplImage *iplImg;
    static IplImage *colorImg[8] = {nullptr, };

    riplImg = frame;
    iplImg = &riplImg;

    CvSize isize = cvSize(iplImg->width, iplImg->height);
    if (colorImg[n] == nullptr)
       colorImg[n] = cvCreateImage(isize, IPL_DEPTH_8U, 3);
    if (iplImg->nChannels == 1)
        cvCvtColor(iplImg, colorImg[n], CV_GRAY2RGB);
    else if (iplImg->nChannels == 3)
        cvCopy(iplImg, colorImg[n]);
    else if (iplImg->nChannels == 4) {
        if (strncmp(iplImg->channelSeq, "BGRA", 4) == 0)
            cvCvtColor(iplImg, colorImg[n], CV_BGRA2RGB);
        else
            cvCvtColor(iplImg, colorImg[n], CV_RGBA2RGB);

        int step = colorImg[n]->widthStep;
        int width = colorImg[n]->width;
        int height = colorImg[n]->height;
        char *u = colorImg[n]->imageData;
