...
void ViewMainPage::ClearU()
{
    ...
    v->document()->setImageInternal(QImage());
...
void ViewMainPage::showContextMenu()
{
    ...
    QAction actionSetROIU("SetROI", this);
    connect(&actionSetROIU, SIGNAL(triggered()), this, SLOT(SetROIU()));
    contextMenu.addAction(&actionSetROIU);
    QAction actionClearU("Clear", this);
    connect(&actionClearU, SIGNAL(triggered()), this, SLOT(ClearU()));
    contextMenu.addAction(&actionClearU);
