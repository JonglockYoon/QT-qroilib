...
//            this, &ControlsDock::showContextMenu);
    connect(mPropertyBrowser, &PropertyBrowser::currentItemChanged,
            this, &ControlsDock::updateActions);

    retranslateUi();
}

void ControlsDock::setDocument(DocumentView *document)
{
    if (mDocument)
...
void ControlsDock::bringToFront()
{
    show();
    raise();
...
void ControlsDock::currentObjectChanged(Object *object)
{
    mPropertyBrowser->setObject(object);

    bool editingTileset = false;//mDocument && mDocument->type() == Document::TilesetDocumentType;
...
void ControlsDock::updateActions()
{
...
//void ControlsDock::addProperty(const QString &name, const QVariant &value)
//{
//    if (name.isEmpty())
//        return;
...
//void ControlsDock::removeProperty()
//{
//    QtBrowserItem *item = mPropertyBrowser->currentItem();
...
//void ControlsDock::renameProperty()
//{
//    QtBrowserItem *item = mPropertyBrowser->currentItem();
//    if (!mPropertyBrowser->isCustomPropertyItem(item))
//        return;
...
//void ControlsDock::renameProperty(const QString &name)
//{
//    if (name.isEmpty())
//        return;
...
void ControlsDock::showContextMenu(const QPoint& pos)
{   
    QtBrowserItem *item = mPropertyBrowser->currentItem();
    if (!mPropertyBrowser->isCustomPropertyItem(item))
        return;
...
bool ControlsDock::event(QEvent *event)
{
    switch (event->type()) {
...
void ControlsDock::retranslateUi()
{
