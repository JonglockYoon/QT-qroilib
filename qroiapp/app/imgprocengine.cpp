...
int CImgProcEngine::CenterOfPlusmarkVerify(RoiObject *pData, IplImage* imageIn, cv::Point2f cpt)
{
    ...
    ////////////////////////
    CBlobResult blobs;
    blobs = CBlobResult(croppedImage, nullptr, 0);	// Use a black background color.
...
int CImgProcEngine::SingleROICircleWithEdge(IplImage* croppedImage, RoiObject *pData, QRectF rect)
{
	...
	// Fill Blob
	CBlobResult blobs;
    blobs = CBlobResult(grayImg, nullptr, 0);	// Use a black background color.
			...
			if (radius >= nMinCircleRadius && radius <= nMaxCircleRadius)
			{
				...
				if (cPerc >= bestCirclePercentage)
				{
					...
					if (imageData[imgStep * (int)center.y + (int)center.x] == 255)
					{
                        ...
                        if (vecRansicCircle.size() > (unsigned int)nMax)
							break;
					}
				}
				else {
...
int CImgProcEngine::SingleROIScrew(IplImage* croppedImage, RoiObject *pData, QRectF rectIn)
{
    ...
    int nThresholdLowValue;
    int nThresholdHighValue;
    int nMinCircleRadius;
    int nMaxCircleRadius;
    IplImage* grayImg1 = nullptr;
    int t = _ProcessValue1;
...
again:
    ...
    if (m_bSaveEngineImg)
    {
        QString str; str.sprintf(("%d_%d_Src.jpg"), retry, 200);
        SaveOutImage(grayImg, pData, str);
    }

    nMinCircleRadius = 30;// nLow;
    nMaxCircleRadius = 200;// nHigh;
    pParam = pData->getParam(("Minimum circle radius"));
    if (pParam)
        nMinCircleRadius = pParam->Value.toDouble();
    ...
    if (m_bSaveEngineImg)
    {
        QString str; str.sprintf(("%d_%d_Threshold.jpg"), retry, 203);
        SaveOutImage(grayImg, pData, str);
    }

    t = _ProcessValue1 + retry;
    NoiseOut(pData, grayImg, t, 212, retry);
    Expansion(pData, grayImg, t, 213, retry);

    if (m_bSaveEngineImg)
    {
        QString str; str.sprintf(("%d_%d_Noise.jpg"), retry, 234);
        SaveOutImage(grayImg, pData, str);
    }

    FilterIncludeLargeBlob(grayImg);
    if (m_bSaveEngineImg)
    {
        QString str; str.sprintf(("%d_%d_SizeFilter.jpg"), retry, 237);
        SaveOutImage(grayImg, pData, str);
    }


    PostNoiseOut(pData, grayImg, -1, 238, retry);
    PostExpansion(pData, grayImg, -1, 239, retry);

    FilterIncludeLargeBlob(grayImg);
    if (m_bSaveEngineImg)
    {
        str.sprintf(("%d_FilterIncludeLargeBlob.jpg"), 240);
        if (pData) SaveOutImage(grayImg, pData, str, false);
    }

    float fillAreaRate = 0.5;
    pParam = pData->getParam(("Fill area rate"));
    if (pParam)
        fillAreaRate = pParam->Value.toDouble() / 100.0;
    ...
    if (nBlobs > 0) {
        ...
        IplImage *image = cvCreateImage(cvSize(r.width, r.height), IPL_DEPTH_8U, 1);
        cvCopy(grayImg, image);
        cvResetImageROI(grayImg);

        if (m_bSaveEngineImg)
        {
            QString str; str.sprintf("%d_%d_holerect.jpg", retry, 241);
            SaveOutImage(image, pData, str);
        }

        double dFullSize = (double)MAX(image->width,image->height);
        dFullSize = dFullSize * dFullSize;
        double dSize = dFullSize * fillAreaRate;
        if (dArea < dSize)
        {
            str.sprintf("SingleROIScrew blob size %.0f < %.0f (NG)", dArea, dSize);
            //g_cLog->AddLog(_LOG_LIST_SYS, str);
            qDebug() << str;
            cvZero(grayImg);
        } else {
            str.sprintf("SingleROIScrew blob size %.0f > %.0f (OK)", dArea, dSize);
            qDebug() << str;
        }
            ...
            if (size > 1)
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    ...
                    if (pCircle->radius > 0)
                    {
                        ...
                        if (m_bSaveEngineImg)
                        {
                            QString str; str.sprintf(("%d_%d_Filter_%d.jpg"), retry, 800, i);
                            SaveOutImage(image, pData, str);
                        }

                        CBlobResult blobs;
                        blobs = CBlobResult(image, nullptr, 0);	// Use a black background color.
                        int blobCount = blobs.GetNumBlobs();
                        double dLargeArea = 0;
                ...
                if (vecCircle.size() > 0)
                {
                    ...
                    if (m_bSaveEngineImg)
                    {
                        ...
                        QString str; str.sprintf(("%d_%d_Result.jpg"), retry, 900);
...
//_done:
    if (m_DetectResult.dRadius > 0.0 && pData->m_vecDetectResult.size() > 0) {
        ...
        cvSet(rstimg, 0);
        cvCircle(rstimg, cvPoint(pRst->pt.x, pRst->pt.y), pRst->dRadius, 255, 2);
