    ...
    while(!stop){
        ...
        try {
            if (!capture.read(frame))
                ...
                int l = theMainWindow->mLeftDock->thresholdLow;
                int h = theMainWindow->mLeftDock->thresholdHigh;
                if (frame.channels()== 3){
                    if (l > 0 || h > 0) {
                        cv::cvtColor(frame, frame1, CV_BGR2GRAY);
                        cv::inRange(frame1, cv::Scalar(l), cv::Scalar(h), frame1);
                ...
                {
                    if (l > 0 || h > 0) {
                        cv::inRange(frame, cv::Scalar(l), cv::Scalar(h), frame1);
                        cv::cvtColor(frame1, RGBframe, CV_GRAY2RGB);
                        img = QImage((const unsigned char*)(RGBframe.data),
                            RGBframe.cols,RGBframe.rows,QImage::Format_RGB888);
                    } else {
