...
#ifdef _SHOW_ERRORS
    QString msg, format = ("Error en CBlobResult: %s");

	switch (errorCode)
	{
	case EXCEPTION_BLOB_OUT_OF_BOUNDS:
        msg.sprintf(format.toStdString().c_str(), "Intentant accedir a un blob no existent");
		break;
	default:
        msg.sprintf(format.toStdString().c_str(), "Codi d'error desconegut");
		break;
	}
