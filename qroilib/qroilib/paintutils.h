 ...
 */
ROIDSHARED_EXPORT QPainterPath roundedRectangle(const QRectF& rect, qreal radius);

/**
 * Generates a pixmap of size @p size, filled with @p color, whose borders have
 * been blurred by @p radius pixels.
 */
ROIDSHARED_EXPORT QPixmap generateFuzzyRect(const QSize& size, const QColor& color, int radius);
 ...
 */
ROIDSHARED_EXPORT QColor adjustedHsv(const QColor& color, int deltaH, int deltaS, int deltaV);

/**
 * Returns a modified version of @p color, where alpha has been set to @p
 * alphaF.
 */
ROIDSHARED_EXPORT QColor alphaAdjustedF(const QColor& color, qreal alphaF);

/**
 * Returns the smallest QRect which contains @p rectF
 */
ROIDSHARED_EXPORT QRect containingRect(const QRectF& rectF);
