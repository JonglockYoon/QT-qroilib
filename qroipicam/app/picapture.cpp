...
{
    int n = mnCam;
//    char buff[theMainWindow->camWidth*theMainWindow->camHeight*3];
//    memset(buff, 128, sizeof(buff));
//    img = QImage((const unsigned char*)buff,theMainWindow->camWidth,theMainWindow->camHeight,QImage::Format_RGB888);
//    emit processedImage(img, seq);
    ...
    */
    ...
    if (capture.isOpened())
    {
        frameRate = theMainWindow->fps;//(int) capture.get(CV_CAP_PROP_FPS);
        //if (frameRate == 0)
        //    frameRate = 10;
        bOpen = true;
        //frameRate = 5; // test
        qDebug() << "OpenCam succ" << frameRate;
        return true;
    }
    else {
        qDebug() << "OpenCam fail";
