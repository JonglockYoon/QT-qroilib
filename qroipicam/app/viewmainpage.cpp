...
{
    return QSize(theMainWindow->camWidth,theMainWindow->camHeight);
}
...
void ViewMainPage::OpenCam(int seq, int nCam)
{
    ...
    if (myCamCapture->OpenCam() == true) {
        ...
        CvSize isize = cvSize(MAX_CAMERA_WIDTH, MAX_CAMERA_HEIGHT);
...
IplImage* ViewMainPage::getIplcolor()
{
    Qroilib::DocumentView *v = currentView();
    const QImage *pimg = v->image();
    if (!pimg)
...
void ViewMainPage::procHSV()
{
    int s = theMainWindow->mLeftDock->sLow;
    int v = theMainWindow->mLeftDock->vLow;

    if (s == 0 && v == 0)
        return;

    IplImage* iplImg = getIplcolor();
    ...
    cvtColor(img_frame, img_hsv, cv::COLOR_BGR2HSV);


    //지정한 HSV 범위를 이용하여 영상을 이진화
    cv::Mat img_mask1;
    cv::inRange(img_hsv, cv::Scalar(0, s, v), cv::Scalar(180, 255, 255), img_mask1);

    IplImage ipl = img_mask1;
    CvSize isize = cvSize(iplImg->width, iplImg->height);
    cvCvtColor(&ipl, colorImg, cv::COLOR_GRAY2BGRA);
    cvAnd(colorImg, outImg, outImg);


    theMainWindow->outWidget("HSV", outImg);
}
